<template>
    <div id="v_SO2">
        <div id="SO2TimeLineChart" :style="{width: '100%', height: '500px', margin:'0 auto'}"></div>
    </div>
</template>
...
export default {
    ...
    created () {
        this.fetchData("北京", "2018-05-01", "2018-05-30", 4);
    },
    methods: {
        drawLinePM25(){
            // 基于准备好的dom，初始化echarts实例
            var myChart = this.$echarts.init(document.getElementById('SO2TimeLineChart'));
            // 绘制图表
            myChart.setOption({
                title: {
                    text: `${this.cityName}SO2变化趋势`,
