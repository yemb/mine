<template>
    <div id="cPM10">
        <div id="comparePM10" :style="{width: '1200px', height: '500px', margin: '20px auto'}"></div>
    </div>
</template>
...
export default {
    ...
    created () {
        this.fetchCity1Data(this.cityOne, this.dateStart, this.dateEnd, 3);
        this.fetchCity2Data(this.cityTwo, this.dateStart, this.dateEnd, 3);
    },
    mounted() {
        // this.drawLine();
        // this.changeCharts();
    ...
    methods: {
        ...
        drawLine() {
            let myChart = this.$echarts.init(document.getElementById('comparePM10'))
            let option = {
                title: {
                    text: `${this.cityOne} VS ${this.cityTwo} PM10变化趋势`,
                    subtext: `平均PM10 ${this.cityOne}:${this.pm10OneAvg} ${this.cityTwo}:${this.pm10TwoAvg}`,
                    x: 'center',
                    textStyle: {
                        fontSize: 24
                    },
