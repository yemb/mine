<template>
    <div id="cSO2">
        <div id="compareSO2" :style="{width: '1200px', height: '500px', margin: '20px auto'}"></div>
    </div>
</template>
...
export default {
    ...
    created () {
        this.fetchCity1Data(this.cityOne, this.dateStart, this.dateEnd, 4);
        this.fetchCity2Data(this.cityTwo, this.dateStart, this.dateEnd, 4);
    },
    mounted() {
        // this.drawLine();
        // this.changeCharts();
    ...
    methods: {
        ...
        drawLine() {
            let myChart = this.$echarts.init(document.getElementById('compareSO2'))
            let option = {
                title: {
                    text: `${this.cityOne} VS ${this.cityTwo} SO2变化趋势`,
                    subtext: `平均SO2 ${this.cityOne}:${this.so2OneAvg} ${this.cityTwo}:${this.so2TwoAvg}`,
                    x: 'center',
                    textStyle: {
                        fontSize: 24
                    },
